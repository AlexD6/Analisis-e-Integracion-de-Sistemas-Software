...
public class SpotifyRemoveController extends HttpServlet {
	
	 private static final long serialVersionUID = -6818025976353856770L;
	 private static final Logger log = Logger.getLogger(SpotifyRemoveController.class.getName());
	 ...
	 protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (accessToken != null && !"".equals(accessToken)) {
			...
			boolean success = spResource.removeSongFromLibrary(trackId);
			String trackName = spResource.getTrackInfo(trackId).getName();
			req.setAttribute("trackName", trackName);
			
			if(success) {
				req.setAttribute("message", "se eliminó de \"Canciones que te gustan\".");
